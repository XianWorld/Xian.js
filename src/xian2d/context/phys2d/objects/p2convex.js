...
P2Convex.prototype.setVertices = function (vs) {
    var vertices = this.vertices,
        wverts = this._vertices,
        normals = this.normals,
        wnorms = this._normals,
    ...
    i = vs.length;
    vertices.length = wverts.length = i;
    normals.length = wnorms.length = i;

    while (i--) {
        vertices[i] = (vertices[i] || new Vec2()).copy(vs[i]);
    }

    if (!P2Convex.validateVertices(vertices)) {
        log.warn("P2Convex.constructor: passed vertices are invalid, creating convex hull from vertices with gift wrapping algorithm");
        vertices = this.vertices = P2Convex.createConvexHull(vertices);
    }
